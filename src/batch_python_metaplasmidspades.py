...
def access_input_container(block_blob_client, container_name, file_path):
    """
    Adds Azure Blob storage container as resource file.
...
def add_tasks(batch_service_client, job_id, input_files, output_container_sas_url):
    ...
    print('Adding {} tasks to job [{}]...'.format(int(len(input_files)/2), job_id))

    tasks = list()

    for idx in range(int(len(input_files)/2)):
        ...
        tasks.append(batch.models.TaskAddParameter(
            ...
            resource_files=[input_files[2*idx], input_files[2*idx+1]],
            output_files=[batchmodels.OutputFile(
                file_pattern=output_folder_path,
...
if __name__ == '__main__':
    ...
    # Access the input files. This is the collection of files that are to be processed by the tasks.
    input_files = [
        access_input_container(blob_client, input_container_name, file_path)
